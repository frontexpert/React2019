...
const SvgIcon = styled.img`
    filter: invert(100%) sepia(0%) saturate(7484%) hue-rotate(303deg) brightness(105%) contrast(92%);
    width: 50px;
    height: 50px;
    ${props => props.size === 'sm' && `
        width: 34px;
...
const NewCoinIcon = ({
    showTether = false, value, defaultFiat, toggleDropdown, opacity = 1, symbol, size,
}) => {
    ...
    if (!!value) {
        ...
        if (src) {
            return <SvgIcon src={src} alt={coinName} size={size} />;
        }
    }
