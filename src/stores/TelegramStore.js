...
class TelegramStore {
    ...
    @observable loginBtnLocation = false; // false: sidemenu, true: popup
    ...
    @action.bound initByTelegramLogin () {
        ...
        this.loginBtnLocation = false;
    };

    @action.bound setLoginBtnLocation (mode) {
        // this.loginBtnLocation = mode;
    };
        ...
        if (!err) {
            ...
            if (err.code === 401 && baseDcID !== requestDcID) {
                ...
                return this.client('auth.exportAuthorization', { dc_id: requestDcID }, { noErrorBox: true })
                    .then(exportedAuth => {
                        this.client('auth.importAuthorization', { id: exportedAuth.id, bytes: exportedAuth.bytes }, {
                            dcID: requestDcID,
                            noErrorBox: true
                        }).then(() => {
    ...
    _login2FA = (twoFactorCode) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let [err, baseDcID] = await to(this.client.storage.get('dc'));
                const { current_salt } = await this.clientWrapper('account.getPassword', {}, baseDcID ? {
                    dcID: baseDcID
                } : {});

                const password_hash = await makePasswordHash(current_salt, twoFactorCode);

                const { user } = await this.clientWrapper('auth.checkPassword', {
                    password_hash,
    ...
    getLastMessage (targetId, isChannel) {
        if (isChannel) {
            const messages = this.messages.filter(m => m.to_id && (m.to_id.chat_id === targetId || m.to_id.channel_id === targetId));
            if (messages.length > 0) {
                messages.sort((a, b) => b.date - a.date);
                return messages[0];
            }
        } else {
            const messages = this.messages.filter(m => m.from_id && m.to_id && m.to_id.user_id && (m.from_id === targetId || m.to_id.user_id === targetId));
            if (messages.length > 0) {
                messages.sort((a, b) => b.date - a.date);
                return messages[0];
            }
                    ...
                    this.chats.map((item, key) => {
                        if (!item.deactivated && !item.left && item._ !== 'chatForbidden') {
                            ...
                            res.push({
                                ...
                                from: lastMessage._ === 'message'
                                    ? lastMessage.out ? 'You: ' : (firstName ? firstName + ': ' : '')
                                    : message,
                                unread,
                                sendCoins: false,
                            ...
                            if (!isEqual(this.lastMessages[item.id], messageToPush)
                                && !this.isFirstFetch
                                && messageToPush
                    ...
                    this.users.map((item, key) => {
                        if (!item.bot && !item.deleted && !item.self) {
                            ...
                            if (lastMessage) {
                                ...
                                if (!isEqual(this.lastMessages[item.id], messageToPush) && !this.isFirstFetch && messageToPush && messageToPush.message && unread) {
                                    console.log('show notification!', messageToPush);
                                    showBrowserNotification(messageToPush.from, messageToPush.message);
                                }
    ...
    getUserInfo = (c_users, userId) => {
        ...
        for (let i = 0; i < c_users.length; i++) {
            if (c_users[i].id === userId) {
                activeUser.name = (c_users[i].first_name ? c_users[i].first_name : '') + ' ' + (c_users[i].last_name ? c_users[i].last_name : '');
    ...
    getUserName = (c_users, userId) => {
        for (let i = 0; i < c_users.length; i++) {
            if (c_users[i].id === userId) {
                return (c_users[i].first_name ? c_users[i].first_name : '') + ' ' + (c_users[i].last_name ? c_users[i].last_name : '');
            }
        }
        return '';
    };
        ...
        for (let i = 0; i < c_users.length; i++) {
            if (c_users[i].id === userId) {
                return c_users[i].first_name
                    ...
                    : c_users[i].last_name
                        ? c_users[i].last_name
                        : c_users[i].username ? c_users[i].username : '';
            }
        }
        return '';
    };
    ...
    @action.bound getHistoryByIdHash (type, id, hash, round, name) {
        ...
        return this.chatHistory(chat, 0, this.limitRows * (round + 1)).then(function (result) {
            // {
            //     type: 'message',
            //     user: {
            //         name: 'CM | Matthew',
    ...
    @action.bound reFetchActivePeerHistory () {
        if (this.activeChat && this.activeChat.type && this.activeChat.id && this.activeChat.hash) {
            return this.getHistoryByIdHash(this.activeChat.type, this.activeChat.id, this.activeChat.hash, 0, this.activeChat.name);
        } else {
            return false;
        }
    }
    ...
    sendReadSignal = async () => {
        if (!this.activePeer.user_id) {
            let [err, res] = await to(this.clientWrapper('channels.readHistory', {
                channel: this.activePeer,
                max_id: 0,
            }));
        } else {
            let [err, res] = await to(this.clientWrapper('messages.readHistory', {
                peer: this.activePeer,
                max_id: 0
            }));
        }
    ...
    joinChannel = async (type, channel_id, access_hash, name) => {
        let [err, res] = await to(this.clientWrapper('channels.joinChannel', {
            channel: {
                _: 'inputChannel',
                channel_id,
                access_hash,
    ...
    leaveChannel = async (channel_id, access_hash) => {
        let [err, res] = await to(this.clientWrapper('channels.leaveChannel', {
            channel: {
                _: 'inputChannel',
                channel_id,
                access_hash,
